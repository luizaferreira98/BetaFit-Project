...
namespace BetaFit.Desktop.UserControls
{
    ...
    public partial class DashboardUserControl : UserControl
    {
        ...
        {
            if (pnlHero == null || pnlHeroMarca == null)
                return;

            pnlHeroMarca.Left =
                pnlHero.ClientSize.Width -
                pnlHeroMarca.Width -
                30;
        }
            ...
            DashboardResponseDto dashboard)
        {
            ...
            // Total de produtos
            lblProdutosValor.Text =
                dashboard.TotalProducts.ToString("00");

            // Total de categorias
            lblCategoriasValor.Text =
                dashboard.TotalCategories.ToString("00");

            // Produtos em destaque
            lblDestaqueValor.Text =
                dashboard.FeaturedProducts.ToString("00");

            // Produtos ativos
            lblAtivosValor.Text =
                dashboard.ActiveProducts.ToString("00");

            // Quantidade de produtos recentes
            var quantidadeRecentes =
                dashboard.RecentProducts?.Count() ?? 0;

            lblListaQuantidade.Text =
                $"{quantidadeRecentes:00} ITENS";
        }

        private void PreencherProdutos(
            IEnumerable<DashboardProductDto>? produtos)
        {
            if (pnlProdutos == null)
                return;
            ...
            try
            {
                ...
                var lista =
                    produtos?
                        ...
                        .Take(5)
                        .ToList()
                    ?? new List<DashboardProductDto>();

                if (lista.Count == 0)
                {
                    pnlProdutos.Controls.Add(
                        CriarVazio());

                    return;
                }

                // Adiciona de trás para frente
                // para manter a ordem visual.
        ...
        private Control CriarLinhaProduto(
            DashboardProductDto produto)
        {
            var linha = new Panel
            {
                Height = 92,
            ...
            var categoria =
                string.IsNullOrWhiteSpace(
                    ...
                    ? produto.Gender.ToString()
                        .ToUpperInvariant()
                    : $"{produto.CategoryName.ToUpperInvariant()}  /  " +
            ...
            var lblData = new Label
            {
                ...
                ForeColor = BetaFitTheme.TextoMuted,
                Text =
                    produto.CreatedAt == default
                        ? "CADASTRO RECENTE"
                        : $"CADASTRADO EM {produto.CreatedAt:dd/MM/yyyy}"
            };
            ...
            var lblStatus = new Label
            {
                ...
                ForeColor =
                    produto.IsActive
                        ? BetaFitTheme.SucessoTexto
                        : BetaFitTheme.TextoMuted,
                Text =
                    produto.IsActive
                        ? "ATIVO"
                        : "INATIVO",
        ...
        private Control CriarImagemProduto(
            DashboardProductDto produto)
        {
            var painel = new Panel
            {
            ...
            string imageUrl)
        {
            ...
            {
                // Mantém o placeholder caso a imagem
                // não possa ser carregada.
            }
        }

        private static void AplicarImagem(
            Panel painel,
            Label placeholder,
            Image imagem)
        {
            if (painel.IsDisposed)
            {
                imagem.Dispose();
                return;
            }
            ...
            bool erro)
        {
            ...
            lblEstado.ForeColor =
                erro
                    ? BetaFitTheme.PerigoTexto
                    : BetaFitTheme.TextoMuted;
        }

        private static string MensagemErroAmigavel(
            Exception ex)
        {
            if (ex is HttpRequestException)
            {
                return
                    "VERIFIQUE SE A API BETAFIT ESTÁ EM EXECUÇÃO.";
            }
