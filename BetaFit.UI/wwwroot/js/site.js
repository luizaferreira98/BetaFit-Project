...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('[data-cpf-mask]').forEach(i=>i.addEventListener('input',()=>{let v=digits(i.value).slice(0,11); if(v.length>9)v=v.replace(/(\d{3})(\d{3})(\d{3})(\d{0,2})/,'$1.$2.$3-$4'); else if(v.length>6)v=v.replace(/(\d{3})(\d{3})(\d{0,3})/,'$1.$2.$3'); else if(v.length>3)v=v.replace(/(\d{3})(\d{0,3})/,'$1.$2'); i.value=v;}));
  document.querySelectorAll('[data-cep-mask]').forEach(i=>i.addEventListener('input',()=>{let v=digits(i.value).slice(0,8); if(v.length>5)v=v.replace(/(\d{5})(\d{0,3})/,'$1-$2'); i.value=v;}));
