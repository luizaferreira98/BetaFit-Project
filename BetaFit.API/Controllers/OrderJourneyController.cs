...
public class OrderJourneyController(BetaFitDbContext db):ControllerBase {
 ...
 [HttpPost("{id:int}/experience")]public async Task<IActionResult> Experience(int id,OrderExperienceDto dto){var error=ReviewPolicy.Validate(dto.Comment,HttpContext.RequestServices.GetRequiredService<IConfiguration>().GetSection("Reviews:BlockedWords").Get<string[]>());if(error!=null)return BadRequest(new{message=error});var o=await db.Orders.FirstOrDefaultAsync(x=>x.Id==id&&x.UserId==UserId);if(o==null)return NotFound();if(o.Status!=OrderStatus.Entregue||o.ExperienceRating!=null)return BadRequest(new{message="Experiência já avaliada ou pedido ainda não entregue."});o.ExperienceRating=dto.Rating;o.ExperienceComment=dto.Comment.Trim();await db.SaveChangesAsync();return NoContent();}
 ...
 [HttpPost("{id:int}/messages")]public async Task<IActionResult> Message(int id,OrderExperienceDto dto){if(!await db.Orders.AnyAsync(o=>o.Id==id&&(o.UserId==UserId||Staff)))return NotFound();var error=ReviewPolicy.Validate(dto.Comment,HttpContext.RequestServices.GetRequiredService<IConfiguration>().GetSection("Reviews:BlockedWords").Get<string[]>());if(error!=null)return BadRequest(new{message=error});db.OrderMessages.Add(new OrderMessage{OrderId=id,UserId=UserId,IsStaff=Staff,Text=dto.Comment.Trim()});await db.SaveChangesAsync();return NoContent();}
}
