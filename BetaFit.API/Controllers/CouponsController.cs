...
public class CouponsController(BetaFitDbContext db):ControllerBase {
 [HttpGet] public async Task<IActionResult> Get()=>Ok(await db.DiscountCoupons.AsNoTracking().OrderByDescending(x=>x.Id).ToListAsync());
 ...
 [HttpPost("{id:int}/toggle")] public async Task<IActionResult> Toggle(int id){var row=await db.DiscountCoupons.FindAsync(id);if(row==null)return NotFound();row.Active=!row.Active;await db.SaveChangesAsync();return NoContent();}
}
